...
void opencl_LM_init(int LM, int cpt,int block)
{
...
#ifdef BENCH_BUILD
		if (c >= 'a' && c <= 'z')
			opencl_DES_u[c] = c & ~0x20;
		else
			opencl_DES_u[c] = c;
#else
		opencl_DES_u[c] = CP_up[c];
#endif
}


void opencl_LM_set_key_self_test(char *key, int index)
{
	unsigned long c;
	unsigned char *dst;
	...
	/*
	if(index == 0) key="ADVANCE";
	if(index == 1) key="RADEON";
	if(index == 2) key="GRAPHIC";
	if(index == 3) key="MAHELI";
	if(index == 4) key="STARSHI";
	if(index == 5) key="LOVELY";
	if(index == 6) key="NAOMI";
	if(index == 7) key="CUTEE";
	if(index == 8) key="STUPID";
	if(index == 9) key="OPENCL";
	if(index == 10) key="NVIDIA";
	if(index == 11) key="EXTREME";
	if(index == 12) key="SECURE";
	...
	if(index == 19) key="MAHELI";
	if(index == 20) key="STARSHI";
	if(index == 21) key="LOVELY";
	if(index == 22) key="NAOMI";
	if(index == 23) key="CUTEE";
	if(index == 24) key="STUPID";
	if(index == 25) key="OPENCL";
	if(index == 26) key="NVIDIA";
	if(index == 27) key="EXTREME";
	if(index == 28) key="SECURE";
...
*/
	...
	*dst = opencl_DES_u[c];
	...
	if (!c) goto fill6;
	*(dst + sizeof(DES_bs_vector) * 8) = opencl_DES_u[c];
	c = (unsigned char)key[2];
	if (!c) goto fill5;
	*(dst + sizeof(DES_bs_vector) * 8 * 2) = opencl_DES_u[c];
	c = (unsigned char)key[3];
	if (!c) goto fill4;
	*(dst + sizeof(DES_bs_vector) * 8 * 3) = opencl_DES_u[c];
	c = (unsigned char)key[4];
	if (!c) goto fill3;
	*(dst + sizeof(DES_bs_vector) * 8 * 4) = opencl_DES_u[c];
	c = (unsigned char)key[5];
	if (!c) goto fill2;
	*(dst + sizeof(DES_bs_vector) * 8 * 5) = opencl_DES_u[c];
	c = (unsigned char)key[6];
	*(dst + sizeof(DES_bs_vector) * 8 * 6) = opencl_DES_u[c];
	return;
fill7:
	dst[0] = 0;
fill6:
...
char *opencl_DES_bs_get_source_LM(WORD *raw)
{
	...
	block = opencl_DES_do_FP(swapped);
...
WORD *opencl_DES_do_FP(WORD in[2])
{
	static WORD out[2];
	int src, dst;
