...
static void check_mask_sha1(struct mask_context *msk_ctx) {
  ...
  /* Assumes msk_ctx -> activeRangePos[] is sorted. Check if any range exceeds sha1 key limit */
	for( i = 0; i < msk_ctx->count; i++)
...
static int cmp_exact(char *source, int count) {
	...
	if(self_test || cmp_out) {
		...
		if(self_test) {
			if (!have_full_hashes){
				HANDLE_CLERROR(clEnqueueReadBuffer(queue[ocl_gpu_id], buffer_out, CL_TRUE, sizeof(cl_uint) * num, sizeof(cl_uint) * 4 * num, res_hashes, 0, NULL, NULL), "failed reading results");
				have_full_hashes = 1;
			}
