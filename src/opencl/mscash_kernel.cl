...
__kernel void mscash_nnn(__global uint *keys,
		     __global ulong *keyIdx,
		     __global uint *outKeyIdx,
		     __global struct mask_context *msk_ctx,
		     __global uint *salt,
	...
	ctr = i = j = k = 0;
	if (rangeNumChars[2]) PUTCHAR(restore, activeCharPos[2], ranges[2 * MAX_GPU_CHARS]);
	if (rangeNumChars[1]) PUTCHAR(restore, activeCharPos[1], ranges[MAX_GPU_CHARS]);
	...
	do {
		do {
			...
			j++;
			PUTCHAR(restore, activeCharPos[1], ranges[j + MAX_GPU_CHARS]);

		} while ( j < rangeNumChars[1]);

		k++;
		PUTCHAR(restore, activeCharPos[2], ranges[k + 2 * MAX_GPU_CHARS]);

		PUTCHAR(restore, activeCharPos[1], ranges[MAX_GPU_CHARS]);
		j = 0;

	} while( k < rangeNumChars[2]);
}

__kernel void mscash_ccc(__global uint *keys,
		     __global ulong *keyIdx,
		     __global uint *outKeyIdx,
		     __global struct mask_context *msk_ctx,
		     __global uint *salt,
	...
	ctr = i = j = k = 0;
	if (rangeNumChars[2]) PUTCHAR(restore, activeCharPos[2], start[2]);
	if (rangeNumChars[1]) PUTCHAR(restore, activeCharPos[1], start[1]);
	...
	do {
		do {
			...
			j++;
			PUTCHAR(restore, activeCharPos[1], (start[1] + j));

		} while ( j < rangeNumChars[1]);

		k++;
		PUTCHAR(restore, activeCharPos[2], (start[2] + k));

		PUTCHAR(restore, activeCharPos[1], start[1]);
		j = 0;

	} while( k < rangeNumChars[2]);
}

__kernel void mscash_cnn(__global uint *keys,
		     __global ulong *keyIdx,
		     __global uint *outKeyIdx,
		     __global struct mask_context *msk_ctx,
		     __global uint *salt,
	...
	ctr = i = j = k = 0;
	if (rangeNumChars[2]) PUTCHAR(restore, activeCharPos[2], ranges[MAX_GPU_CHARS]);
	if (rangeNumChars[1]) PUTCHAR(restore, activeCharPos[1], ranges[0]);
	...
	do {
		do {
			...
			j++;
			PUTCHAR(restore, activeCharPos[1], ranges[j]);

		} while ( j < rangeNumChars[1]);

		k++;
		PUTCHAR(restore, activeCharPos[2], ranges[k + MAX_GPU_CHARS]);

		PUTCHAR(restore, activeCharPos[1], ranges[0]);
		j = 0;
